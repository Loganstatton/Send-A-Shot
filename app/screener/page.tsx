  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm" style={{ color: 'var(--text-faint)' }}>Every tracked artist, ticker-style — score momentum, investment, and return in one screen.</p>
      </div>
      ...
      {rows.length > 0 && (
        <div className="card overflow-x-auto">
          <table className="w-full text-sm min-w-[860px]" style={{ borderCollapse: 'collapse' }}>
            <thead className="text-left">
              <tr>
                ...
                <th className="font-normal pb-2" style={{ color: 'var(--text-faint)' }}>Trend</th>
                <th className="font-normal pb-2 text-right" style={{ color: 'var(--text-faint)' }}>Score</th>
                <th className="font-normal pb-2 text-right" style={{ color: 'var(--text-faint)' }}>Change</th>
                <th className="font-normal pb-2 text-right" style={{ color: 'var(--text-faint)' }}>Invested</th>
                <th className="font-normal pb-2 text-right" style={{ color: 'var(--text-faint)' }}>Commission</th>
                <th className="font-normal pb-2 text-right" style={{ color: 'var(--text-faint)' }}>ROI</th>
            ...
            <tbody>
              {rows.map((row) => {
                ...
                const changeColor = row.changeAbs > 0 ? 'var(--up)' : row.changeAbs < 0 ? 'var(--down)' : 'var(--text-faint)';
                return (
                  <tr key={row.artist.id} style={{ borderTop: '1px solid var(--border-soft)' }}>
                    <td className="py-2.5 pr-3">
                      <Link href={`/artists/${row.artist.id}`} className="font-semibold hover:underline">
                        {row.artist.name}
                      </Link>
                      {row.artist.genre && <div className="text-xs" style={{ color: 'var(--text-faint)' }}>{row.artist.genre}</div>}
                    </td>
                    <td className="py-2.5 pr-3 whitespace-nowrap" style={{ color: 'var(--text-muted)' }}>{STAGE_LABELS[row.artist.stage]}</td>
                    <td className="py-2.5 pr-3">
                    ...
                    <td className="num py-2.5 pr-3 text-right font-semibold">{row.score.toFixed(1)}</td>
                    <td className="num py-2.5 pr-3 text-right font-medium whitespace-nowrap" style={{ color: changeColor }}>
                      {row.hasComparison ? (
                        <>
                    ...
                    <td className="num py-2.5 pr-3 text-right whitespace-nowrap">{formatCents(row.totalInvestedCents)}</td>
                    <td className="num py-2.5 pr-3 text-right whitespace-nowrap">{formatCents(row.totalCommissionCents)}</td>
                    <td
                      className="num py-2.5 pr-3 text-right whitespace-nowrap font-medium"
                      style={{ color: row.roiPct == null ? 'var(--text-faint)' : row.roiPct >= 0 ? 'var(--up)' : 'var(--down)' }}
                    >
                      {row.roiPct != null ? `${row.roiPct > 0 ? '+' : ''}${row.roiPct}%` : '—'}
                    </td>
                    <td className="py-2.5 whitespace-nowrap">{status.emoji} {status.label}</td>
                  </tr>
                );
              })}
            </tbody>
