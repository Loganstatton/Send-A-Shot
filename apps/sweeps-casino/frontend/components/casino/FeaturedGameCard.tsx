...
import { ArrowRight, Shield, Lock } from "@/components/ui/icons";
...
export function FeaturedGameCard({ game, tagline }: { game: Game; tagline?: string }) {
  ...
  const href = isOriginal
    ? `/casino/originals/${game.slug}`
    : isPlayableSlotGame
      ? `/casino/slots/${game.slug}`
      : `/casino/games/${game.slug}`;
  const label = tagline ?? (isOriginal ? "Exclusive Vaultline Original" : "Featured Game");

  // Mirrors GameTile's gating: a featured SLOTS card with no real engine
  // behind it never navigates anywhere on tap.
  if (!playable) {
    return (
      <div
        className="group relative block h-[220px] w-full cursor-default overflow-hidden rounded-2xl border border-border/60 shadow-card-lift sm:h-[280px]"
        style={isOriginal || usesCategoryArt ? undefined : { background: tileGradient(seed) }}
        role="group"
        aria-label={`${game.name} — coming soon, not yet playable`}
      >
        <div className="absolute inset-0">{usesCategoryArt && <CategoryArt category={game.category as "SLOTS" | "TABLE_GAMES" | "LIVE_CASINO" | "GAME_SHOWS"} seed={seed} />}</div>
        <div className="absolute inset-0 bg-gradient-to-r from-black/90 via-black/60 to-black/25" />
        <div className="relative z-10 flex h-full max-w-md flex-col justify-center gap-2.5 p-6 sm:gap-3 sm:p-10">
          <span className="inline-flex w-fit items-center gap-1.5 rounded-full border border-border bg-black/40 px-2.5 py-1 text-[10px] font-bold uppercase tracking-[0.18em] text-white/70 backdrop-blur-sm">
            <Lock className="h-3 w-3" />
            Coming Soon
          </span>
          <h3 className="text-2xl font-extrabold leading-tight text-white/90 drop-shadow-lg sm:text-3xl">{game.name}</h3>
          <p className="text-xs text-white/50 sm:text-sm">{game.provider}</p>
        </div>
      </div>
    );
  }
